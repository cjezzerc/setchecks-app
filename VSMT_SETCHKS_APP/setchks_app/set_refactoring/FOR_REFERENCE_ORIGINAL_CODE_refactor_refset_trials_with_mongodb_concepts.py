...
import refset_module_modified_mongodb
# import msaccess
...
from concept_module_mongodb import ConceptsDict
...
# refsets=refset_module_modified_mongodb_mongodb.RefsetCollection()
# for refset_id, description, sctrule, meta in msaccess_list:
#     print("===>>>>",refset_id, description)
#     refsets.append(refset=refset_module_modified_mongodb.Refset(refset_name=refset_id, refset_description=description, clause_set_string=sctrule, metadata_column=meta))
# refsets.process_filters(concepts=concepts)

refsets=refset_module_modified_mongodb.RefsetCollection()
...
metadata_column='----------- ----------'
trial_refset=refset_module_modified_mongodb.Refset(clause_set_string=trial_SCT_RULE, metadata_column=metadata_column, refset_name='trial')
...
for refset_name in refset_list_to_refactor:
    ...
    for clause in refset.clause_based_rule.clauses:
        if clause.clause_type=="include" and clause.sticky==True:
            ...
            concepts_captured_by_clause=set(refset_module_modified_mongodb.ClauseMembershipAnalysis(clause=clause, concepts=concepts).members)
    ...
    for cbc in perfect_fit_incl_cbcs:
        ...
        clause=refset_module_modified_mongodb.Clause(clause_string)
        refactored_query.append(clause)
    ...
    for clause in refactored_query:
        concepts_captured_by_clause=refset_module_modified_mongodb.ClauseMembershipAnalysis(clause=clause, concepts=concepts).members
    ...
    while not terminate_condition_reached:
        ...
        if cbcs_list_not_empty and winner_cbc.score >= fit_threshold-0.001: # 0.001 is to allow for rounding error; on first pass 0.899999999999 can occur
            ...
            clause=refset_module_modified_mongodb.Clause(winner_cbc.clause_string)
    ...
    for concept_id in trimmed_refset_members_set:
        ...
        refactored_query.append(refset_module_modified_mongodb.Clause(clause_string))
    ...
    for clause in refactored_query:
        total_inclusions.update(set(refset_module_modified_mongodb.ClauseMembershipAnalysis(clause=clause, concepts=concepts).members))
    ...
    for cbc in all_excl_cbcs:
        ...
        clause=refset_module_modified_mongodb.Clause("-"+cbc.clause_string)
        refactored_query.append(clause)
        total_exclusions.update(set(refset_module_modified_mongodb.ClauseMembershipAnalysis(clause=clause, concepts=concepts).members))
    ...
    for i_clause, clause in enumerate(refactored_query):
        clause_members_sets.append(set(refset_module_modified_mongodb.ClauseMembershipAnalysis(clause=clause, concepts=concepts).members))
    ...
    refactored_refset=refset_module_modified_mongodb.Refset(clause_set_string=SCT_RULE, metadata_column=refset.metadata_column)
    # Switch OFF filters  
    refactored_refset.apply_filters=refactor_apply_filters

    refsets.append(refset=refactored_refset)
    refactored_rfa=refset_module_modified_mongodb.RefsetMembershipAnalysis(refset=refactored_refset, concepts=concepts)
    ...
    for set_name in ["include_clauses_only_in_original","include_clauses_only_in_refactored","exclude_clauses_only_in_original","exclude_clauses_only_in_refactored"]:
        ...
        if set_contents:
            ...
            for clause_string in set_contents:
                ...
                print("%34s: %20s %s " % (set_name, clause_string, concepts[base_concept_id].pt))
        else:
