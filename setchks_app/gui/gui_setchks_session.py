...
def get_setchk_session(session=None):
    ...
    if (
        "setchks_session" in session.keys() and session["setchks_session"] != None
    ):  # grab setchks_session from session variable if it is in there
        ...
        setchks_session.email = session["jwt_token"]["userinfo"][
            "email"
        ]  # make sure login email stays current in odd cases where e.g.
