...
def select_and_run_checks():
    ...
    for sc in available_setchks:
        ...
        if (
            "ALL" in this_setchk.setchk_data_entry_extract_types
            or setchks_session.data_entry_extract_type
            in this_setchk.setchk_data_entry_extract_types
        ) and (
    ...
    if (setchks_session.processing_status == "2_PREPROCESSING") and (
        setchks_session.preprocessing_done
    ):
        setchks_session.processing_status = "3_CHECKS_RUNNING"
        processing_status_changed_this_visit = True
    elif (setchks_session.processing_status == "3_CHECKS_RUNNING") and (
        setchks_session.all_CHKXX_finished
    ):
        setchks_session.processing_status = "4_CREATING_REPORT"
        processing_status_changed_this_visit = True
    elif (setchks_session.processing_status == "4_CREATING_REPORT") and (
        setchks_session.excel_file_available
    ):
    ...
    if (
        "do_preprocessing" in request.args
        and setchks_session.processing_status == "1_CHECKS_READY_TO_RUN"
        and (setchks_jobs_manager is None or setchks_jobs_manager.jobs_running == False)
    ):

        setchks_session.time_started_processing = datetime.datetime.now().strftime(
            "%d_%b_%Y__%H_%M_%S"
        )
        ...
        if not setchks_session.preprocessing_failed:
            run_in_rq = True
            if run_in_rq:
                setchks_jobs_manager = SetchksJobsManager(
    ...
    if (
        setchks_session.processing_status == "3_CHECKS_RUNNING"
        and processing_status_changed_this_visit
    ):
    ...
    if (
        ...
        if not setchks_session.excel_file_generation_failed:
            ...
            excel_filename = "%s/%s_vsmt_setchecks_output_%s.xlsx" % (
                user_tmp_folder,
                name_prefix,
                datetime.datetime.now().strftime("%d_%b_%Y__%H_%M_%S"),
            )
            ...
            setchks_app.mgmt_info.handle_summary_info.store_summary_dict_to_db(
                setchks_session=setchks_session
            )
            setchks_app.mgmt_info.handle_setchks_session.store_setchks_session(
                setchks_session=setchks_session
            )
            run_in_rq = True
            if run_in_rq:
    ...
    if (
        ...
        setchks_app.mgmt_info.handle_excel_file.store_excel_file(
            setchks_session=setchks_session
        )

    if (
        "download_report" in request.args
        and setchks_session.processing_status == "5_REPORT_AVAILABLE"
    ):
