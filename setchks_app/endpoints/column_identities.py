...
def column_identities():
    ...
    if "uploaded_file" in request.files:
        if setchks_session.load_file_behaviour == "DEFAULT_SETTINGS":
            ...
            setchks_session = gui_setchks_session.get_setchk_session(session)
        multisheet_flag = setchks_session.load_data_into_matrix(
            data=request.files["uploaded_file"],
            upload_method="from_file",
        ...
        if (
            ...
            or (
                setchks_session.columns_info.ncols
                != len(setchks_session.data_as_matrix[0])
            )
    ...
    if len(request.form.keys()) != 0:
        ...
        success_flag, message = ci.set_column_type(
            icol=icol, requested_column_type=requested_column_type
        )
        logger.debug(
            "Type change attempt: %s %s %s %s"
            % (icol, requested_column_type, success_flag, message)
        )
    ...
    if not too_many_rows:
        if setchks_session.marshalled_rows == []:
            ...
            ]:  # The marshalled_rows list does NOTinclude the header row
                mr = MarshalledRow(
                    row_data=row, columns_info=setchks_session.columns_info
                )
                setchks_session.marshalled_rows.append(mr)
            setchks_session.column_content_assessment.assess(
                marshalled_rows=setchks_session.marshalled_rows
            )
    else:
    ...
    if setchks_session.columns_info is not None:
        column_type_labels = [
            type_labels[x] for x in setchks_session.columns_info.column_types
        ]
    else:
