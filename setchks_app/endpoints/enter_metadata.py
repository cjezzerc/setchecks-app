...
def enter_metadata():
    ...
    )  # remember this in case changes in next sections
    current_sct_version_b = (
        setchks_session.sct_version_b
    )  # remember this in case changes in next sections
    ...
    if "select_sct_version" in request.form:
        ...
        setchks_session.sct_version = setchks_session.available_sct_versions[
            int(request.form["select_sct_version"]) - 1
        ]
    ...
    if "pointNumber" in request.form:
        ...
        setchks_session.sct_version = setchks_session.available_sct_versions[
            int(request.form["pointNumber"])
        ]

    # if reach here via click on versions dropdown (b)
    if "select_sct_version_b" in request.form:
        # print("===>>>>", request.form['select_sct_version'])
        setchks_session.sct_version_b = setchks_session.available_sct_versions[
            int(request.form["select_sct_version_b"]) - 1
        ]
    ...
    if "pointNumber_b" in request.form:
        # print("===>>>> pointNumber=", request.form['pointNumber'])
        setchks_session.sct_version_b = setchks_session.available_sct_versions[
            int(request.form["pointNumber_b"])
        ]

    if "data_entry_extract_type" in request.form:
        setchks_session.data_entry_extract_type = request.form[
    ...
    if (
        setchks_session.sct_version != current_sct_version
    ):  # if have changed sct_version
        setchks_session.reset_analysis()  # throw away all old results

    if (
        setchks_session.sct_version_b != current_sct_version_b
    ):  # if have changed sct_version_b
        setchks_session.reset_analysis()  # throw away all old results

    timeline_data_json, timeline_layout_json, timeline_info_json = (
        graphical_timeline.create_graphical_timeline(
    ...
    if setchks_session.sct_version_mode == "DUAL_SCT_VERSIONS":
        timeline_data_json_b, timeline_layout_json_b, timeline_info_json_b = (
            graphical_timeline.create_graphical_timeline(
