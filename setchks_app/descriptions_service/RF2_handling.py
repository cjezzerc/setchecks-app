...
def create_collection_from_RF2_file(
    ...
    if collection.name in db.list_collection_names():
        if delete_if_exists == True:
            ...
            return (
                False,
                "Collection %s already exists; use delete_if_exists=True to overwrite"
                % collection_name,
            )
    ...
    if data_type == "descriptions":
        ...
        for line in open(RF2_filename2).readlines()[1:]:
            ...
            if refsetId in [
                "999001261000000100",
                "999000691000001104",
            ]:  # NHS realm lang refset (clinical part + pharmacy part)
                ...
                if acceptability_active_status == "1":
                    ...
                    assert (
                        ...
                        or (acceptabilities[referenceComponentId] == acceptability_temp)
                        #    (referenceComponentId not in acceptabilities)
    ...
    for line in open(RF2_filename).readlines()[1:]:
        ...
        if data_type == "descriptions":
            ...
            if desc_id in acceptabilities or (
                active_status == "0" and desc_id in inactive_acceptabilities
            ):  # (only add description if not "unnacceptable" i.e must be (active) in lang refset)
