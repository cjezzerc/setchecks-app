...
class DescriptionsService:
    ...
    def delete_database(self):
        if self.data_type == "descriptions":
            get_mongodb_client.get_mongodb_client().drop_database(
    ...
    def get_trud_releases_info(self):
        if self.data_type == "descriptions":
            url = (
                "https://isd.digital.nhs.uk/trud/api/v1/keys/%s/items/1799/releases"
                % (os.environ["TRUDAPIKEY"],)
            )
        elif self.data_type in ["hst", "qt"]:
            url = (
                "https://isd.digital.nhs.uk/trud/api/v1/keys/%s/items/276/releases"
                % (os.environ["TRUDAPIKEY"],)
            )
        else:
            return None
    ...
    def pull_release_from_trud(self, date_string=None):
        ...
        if date_string not in trud_dict:
            logging.debug(
                f"{date_string} does not have a trud release for {self.data_type}"
            )
            return
        ...
        if self.data_type == "descriptions":
            ...
            glob_pattern2 = (
                extract_dir + "/*/Snapshot/Refset/Language/der2_cRefset_Language*"
            )
        elif self.data_type == "hst":
    ...
    def get_data_about_description_id(
        self, description_id=None, date_string=None, sct_version=None
    ):
        ...
        data_found = list(
            self.db[collection_name].find({"desc_id": str(description_id)})
        )
        if data_found == []:
            return None
        else:
    ...
    def get_data_about_concept_id(
        self, concept_id=None, date_string=None, sct_version=None
    ):
        ...
        collection_name = self.make_collection_name(date_string=date_string)
        data_found = list(
            self.db[collection_name].find({"concept_id": str(concept_id)})
        )
        return data_found
    ...
    def get_hst_data_from_old_concept_id(self, old_concept_id=None, sct_version=None):
        ...
        data_found = list(
            self.db[collection_name].find({"old_concept_id": str(old_concept_id)})
        )
        return data_found
    ...
    def get_hst_data_from_new_concept_id(self, new_concept_id=None, sct_version=None):
        ...
        collection_name = self.make_collection_name(date_string=sct_version.date_string)
        data_found = list(
            self.db[collection_name].find({"new_concept_id": str(new_concept_id)})
        )
        return data_found
    ...
    def db(self):  # only connect the first time it is needed and then store it
        if self._db is None:
            if self.data_type == "descriptions":
                self._db = get_mongodb_client.get_mongodb_client()[
                    "descriptions_service"
                ]
            elif self.data_type == "hst":
