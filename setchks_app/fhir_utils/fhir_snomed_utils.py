    ...
    for parameter in parameters.parameter:
        ...
        if name not in ["property", "designation"]:
            ...
            if k in [
                ...
                "609096000",
                "123005000",
            ]:  # last two are role-group and part-of
                if k not in concept_attributes:
                    concept_attributes[k] = (
                        []
                    )  # as do not know a priori how many entries may be seen for a particular 'property'
                    # then initialise all as lists
                if type(v) is not list:
                    concept_attributes[k].append(int(v))
                else:
                    concept_attributes[k].append(
                        v
                    )  # still need to deal with fact that these sublists contain strings
            else:
                assert k not in concept_attributes  # these keys should only occur once
                concept_attributes[k] = v
        else:  # i.e. name == designation
            concept_attributes["designation"].append(
                get_designation_language_use_value(parameter.part)
            )
        # print("--------------------------------------------------")
    ...
    for d in part:
        ...
        if d.name == "subproperty":
            if value is None:
                value = []
            value.append(
                get_property_name_and_value(d.part)
            )  # recursive call, but only expect it to go one deep
    return [name, value]
