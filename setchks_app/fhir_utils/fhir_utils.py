...
def repr_resource(
    resource=None, repr_strings=None, indent=0, object_representation="resource"
):
    # print(resource)
    ...
    padding_string = " " * (
        20 - indent
    )  # this form keeps colons aligned - not sure which prefer
    if type(resource) == list:
        for i_thing, thing in enumerate(resource):
            repr_resource(
                ...
                indent=indent + 3,
                object_representation=object_representation + "[" + str(i_thing) + "]",
            )
        ...
        for putative_tuple in resource:
            ...
            if thing_value is not None:
                ...
                if (
                    (t is not str)
                    and iterable(thing_value)
                    and (
                        not (type(thing_value) == list and type(thing_value[0]) == str)
                    )
                ):
                    ...
                    repr_strings.append(
                        ...
                        % (
                            indent_string,
                            object_representation + "." + thing_name + decorator,
                            padding_string,
                        )
                    )
                    ...
                    repr_resource(
                        ...
                        repr_strings=repr_strings,
                        indent=indent + 3,
                        object_representation=object_representation + "." + thing_name,
                    )
                else:
                    repr_strings.append(
                        "%s %-50s %s: %s"
                        % (
                            indent_string,
                            object_representation + "." + thing_name,
                            padding_string,
                            thing_value,
                        )
                    )
