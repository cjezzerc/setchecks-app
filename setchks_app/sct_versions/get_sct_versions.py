...
def get_sct_versions_on_ontoserver():
    ...
    available_sct_versions_on_ontoserver = [
        be.resource.dict()["version"] for be in bundle.entry
    ]
    available_sct_versions_on_ontoserver = [
        sct_version.SctVersion(formal_version_string=x)
        for x in available_sct_versions_on_ontoserver
    ]
    available_sct_versions_on_ontoserver.sort(
        key=get_sortable_date_part, reverse=True
    )  # mild overkill since as it stands sorting on
...
def get_sct_versions_available_in_app(descriptions_service=None):
    all_available_sct_versions = {
        x.date_string: x for x in get_sct_versions_on_ontoserver()
    }
    available_sct_versions_in_app = []
    hst_dict = (
        descriptions_service.check_whether_releases_on_ontoserver_have_collections()
    )
    for sct_version, hst_exists in hst_dict.items():
        if hst_exists:  # only make sct_version available if has an HST
            available_sct_versions_in_app.append(
                all_available_sct_versions[sct_version]
            )
