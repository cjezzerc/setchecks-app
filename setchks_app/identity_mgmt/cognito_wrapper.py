...
def auth_required(f):
    @functools.wraps(
        f
    )  # not quite sure why added this but now if remove it get overwriting existing endpoint error
    def wrap2(*args, **kwargs):
        logger.debug(f"AUTHORISATION_LOCAL={os.environ['AUTHORISATION_LOCAL']}")
        if (
            os.environ["DEPLOYMENT_ENV"] == "LOCAL"
            and os.environ["AUTHORISATION_LOCAL"].lower() == "false"
        ):  # possibly skip authorisation if running locally
            session["jwt_token"] = {}
        ...
        if have_token:
            ...
            if time.time() < jwt_token["exp"]:
                ...
                session["jwt_token"] = get_token_from_refresh_token(
        ...
        if authorized:
            ...
            if os.environ["DEPLOYMENT_ENV"] == "AWS":
                if (
                    os.environ["ENV"] != "demo"
                ):  # only let vsmt_internal into environments other than demo
            ...
            if put_up_closed_message:
                if (os.environ["DEPLOYMENT_ENV"] != "LOCAL") and (
                    os.environ["ENV"] == "demo"
                ):
            ...
            if level_allowed:
                ...
                return (
                    "You are not authorised to access this endpoint in this environment"
                )
...
def auth_required_admin(f):
    ...
    def wrap2(*args, **kwargs):
        ...
        if (
            os.environ["DEPLOYMENT_ENV"] == "LOCAL"
            and os.environ["AUTHORISATION_LOCAL"].lower() == "false"
        ):  # skip authorisation if running locally
            session["jwt_token"] = {}
            session["jwt_token"]["email"] = "local_user"
            return f(*args, **kwargs)
