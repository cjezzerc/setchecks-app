...
def get_set_of_incl_cbcs_based_on_all_ancestors(
    ...
    for i_cbc, concept_id in enumerate(all_inclusion_candidate_base_concept_ids):
        debug_print(
            "Getting cbc %s ( %s of %s )"
            % (concept_id, i_cbc, len(all_inclusion_candidate_base_concept_ids))
        )
        cbc = CandidateBaseConcept(
            concept_id=concept_id,
            concepts=concepts,
            target_members=valset_membership_analysis.final_inclusion_list,
        )
...
def purge_poor_quality_incl_cbcs(all_incl_cbcs=None):
    ...
    for cbc in all_incl_cbcs:
        ...
        if (
            (cbc.n_desc_plus_self_not_in_membership == 1)
            and (cbc.is_in_membership is False)
            and (cbc.n_children_all == 1)
        ):
            to_be_purged_reason += "reason2 "
        if (float(cbc.n_children_in_membership) / cbc.n_children_all) < 0.5:
            to_be_purged_reason += "reason3 "
        if ((float(cbc.n_children_in_membership) / cbc.n_children_all) < 0.6) and (
            cbc.n_children_all > 9
        ):
            to_be_purged_reason += "reason4 "
        if cbc.concept_id == 996581000000109:
            debug_print(
                f"PROBLEM ID DATA nc:{cbc.n_children_all} ncm:{cbc.n_children_in_membership}"
            )
...
def purge_perfect_fit_incl_cbcs_subsumed_by_another_perfect_fit_cbc(all_incl_cbcs=None):
    ...
    for cbc in all_incl_cbcs:
        if cbc.is_perfect_fit and (
            cbc.concept.ancestors.isdisjoint(concepts_ids) is False
        ):
            to_be_purged_set.add(cbc)
    debug_print(
        "Purging %s candidate base concepts that are perfectly subsumed by an ancestor"
        % len(to_be_purged_set)
    )
    t2 = time.time() - start_time2
    debug_print(
        "Purging redundant prefect fit clauses took (in seconds)",
...
def insert_perfect_fits_into_refactored_query(
    perfect_fit_incl_cbcs=None, refactored_query=None
):
...
def trim_the_working_set(
    ...
    for clause in refactored_query:
        concepts_captured_by_clause = ClauseMembershipAnalysis(
            clause=clause, concepts=concepts
        ).members
        concepts_captured_by_refactored_query.update(concepts_captured_by_clause)
    n_members_captured_by_concepts = len(
        trimmed_valset_members_set.intersection(concepts_captured_by_refactored_query)
    )
...
def iterate_to_find_best_imperfect_fit_clauses_to_add(
    ...
    if (
        len(trimmed_valset_members_set) == 0
    ):  # this is a specific line in DMWB code; not sure completely in line with logic of some of the comment about
    ...
    while not terminate_condition_reached:
        ...
        debug_print(
            "\n\nIteration %s :  LSize=%s Fit=%s Z=%s"
            % (iteration, list_size, fit_threshold, zoom)
        )
        ...
        cbcs_sorted_by_score = sorted(
            imperfect_fit_incl_cbcs, key=lambda cbc: cbc.score, reverse=True
        )
        ...
        if (
            cbcs_list_not_empty and winner_cbc.score >= fit_threshold - 0.001
        ):  # 0.001 is to allow for rounding error; on first pass 0.899999999999 can occur
            winner_exists_at_current_zoom = True
            debug_print(
                "Winner is %s with score %s"
                % (winner_base_concept.concept_id, winner_cbc.score)
            )
            if len(cbcs_sorted_by_score) > 1:
                debug_print(
                    "Secondplace is %s with score %s"
                    % (
                        cbcs_sorted_by_score[1].concept_id,
                        cbcs_sorted_by_score[1].score,
                    )
                )
            else:
            ...
            n_records_to_be_removed_from_trimmed_set = len(
                trimmed_valset_members_set.intersection(concepts_captured_by_clause)
            )
            ...
            concepts_captured_that_will_need_exclusion = set(
                concepts_captured_by_clause
            ).difference(set(valset_membership_analysis.final_inclusion_list))
            ...
            for cbc in imperfect_fit_incl_cbcs:
                if (
                    cbc.concept_id in winner_base_concept.descendants
                    or cbc == winner_cbc
                ):
                    to_be_purged_set.add(cbc)
            imperfect_fit_incl_cbcs.difference_update(to_be_purged_set)
            debug_print(
                "Purged %s cbcs as descendants of winner or the winner itself"
                % len(to_be_purged_set)
            )

            # Look to see if although found a winner things are being found in clauses that are too small (not convinced by logic here)
            fold_left = float(n_in_valset) / (
                n_records_to_be_removed_from_trimmed_set + 1
            )  # I am not convinced by this section ? intended definition of ClauseCoverage in original code
            if fold_left > 1000:
                new_fit_threshold = round(fit_threshold * 0.9, 3)
                if (new_fit_threshold > min_fit_threshold) and (
                    new_fit_threshold < fit_threshold
                ):
                    debug_print(
                        "Winning clause too small (%s of remaining records covered removed) - detuning"
                        % n_records_to_be_removed_from_trimmed_set
                    )
            ...
            if (
                (zoom <= max_clause_count)
                and (zoom <= list_size)
                and (zoom != round((zoom * 1.1) + 0.5, 0))
            ):
                ...
                if (new_fit_threshold > min_fit_threshold) and (
                    ...
                    # fit_threshold = new_fit_threshold
                    fit_threshold = round(
                        new_fit_threshold * 0.95, 3
                    )  # added 25oct23 after discussion with Jeremy Rogers
                    ...
                    debug_print(
                        "TERMINATING: Can't detune inclusion weighting function any further"
                    )
                    debug_print(
                        "Next best fitting candidate has score of ", new_fit_threshold
                    )
...
def add_single_concept_clauses_for_unaccounted_for_concepts(
    ...
    for concept_id in trimmed_valset_members_set:
        if concepts[concept_id].descendants == set():
            ...
            operator = (
                "="  # else have decendants so use "=" so do not include non members
            )
...
def assert_everything_now_included(
    ...
    for clause in refactored_query:
        total_inclusions.update(
            set(ClauseMembershipAnalysis(clause=clause, concepts=concepts).members)
        )
    ...
    if not inclusion_check:
        debug_print(
            "ERROR: Exiting as refactored_query does not account for everything"
        )
        sys.exit()
...
def get_set_of_excl_cbcs_based_on_all_ancestors(
    ...
    debug_print(
        "Collated list of %s cbcs in all_exclusion_candidate_base_concept_ids"
        % len(all_exclusion_candidate_base_concept_ids)
    )
    ...
    n_to_remove = len(
        all_exclusion_candidate_base_concept_ids.intersection(
            all_inclusion_candidate_base_concept_ids
        )
    )
    debug_print("Discarding %s cbcs because also an inclusion ancestor" % n_to_remove)
    all_exclusion_candidate_base_concept_ids.difference_update(
        all_inclusion_candidate_base_concept_ids
    )
    debug_print(
        "Now have list of %s cbcs in all_exclusion_candidate_base_concept_ids"
        % len(all_exclusion_candidate_base_concept_ids)
    )
    ...
    for concept_id in all_exclusion_candidate_base_concept_ids:
        ...
        cbc = CandidateBaseConcept(
            concept_id=concept_id,
            concepts=concepts,
            target_members=required_exclusions_set,
            must_avoid_set=full_valset_members_set,
        )
...
def purge_poor_quality_excl_cbcs(all_excl_cbcs=None):
    ...
    for cbc in all_excl_cbcs:
        ...
        if (
            cbc.n_desc_plus_self_all - cbc.n_desc_plus_self_in_membership
        ) > cbc.n_desc_plus_self_in_membership:  # (line DMWB1040)
            to_be_purged_reason += "reason3 "
        # TempDB.Execute "DELETE FROM greedy_ancestors_x WHERE TRUENEG = 1 AND INCLUDE = FALSE AND ALLCHILDREN = 1;"
        if (
            cbc.n_desc_plus_self_not_in_membership == 1
            and cbc.is_in_membership == True
            and cbc.n_children_all == 1
        ):  # line DMWB1037
            to_be_purged_reason += "reason4 "
        if to_be_purged_reason != "":
...
def purge_excl_cbcs_subsumed_by_excl_cbc(all_excl_cbcs=None):
    ...
    debug_print(
        "Purging %s excl_cbcs that are perfectly subsumed by an ancestor"
        % len(to_be_purged_set)
    )
...
def purge_excl_clauses_that_would_hit_valset_members(all_excl_cbcs=None):
    ...
    for cbc in all_excl_cbcs:
        ...
        assert (
            cbc.is_perfect_fit
        )  # This whole function should I think never find anything that hits a valset member because
...
def add_single_concept_excl_clauses_for_unaccounted_for_exclusions(
    ...
    debug_print(
        "Adding clauses to cover residue of %s concepts remaining in unaccounted_for_exclusions"
        % len(unaccounted_for_exclusions)
    )
    ...
    for i_clause2, clause2 in enumerate(refactored_query):
        ...
        for i_clause1, clause1 in enumerate(refactored_query):
            if clause1.clause_base_concept_id in concepts:
                if (i_clause1 != i_clause2) and (
                    clause1.clause_type == clause2.clause_type
                ):  # NB in DMWB INCLUDE meanes include/exclude for a cluase; but for a (base) concept it means is the concept
                    # in the valset (I think)
                    if (i_clause2 > i_clause1) and (
                        clause1.clause_string == clause2.clause_string
                    ):  # check for duplicate
                        purge2 = True
                    ...
                    if clause1.clause_base_concept_id == clause2.clause_base_concept_id:
                        ...
                        if (clause2.clause_operator == "<") and (
                            clause1.clause_operator == "<<"
                        ):
                            purge2 = True
                    ...
                    if (clause1.clause_operator in ["<", "<<"]) and (
                        clause2.clause_base_concept_id
                        in concepts[clause1.clause_base_concept_id].descendants
                    ):
                        purge2 = True
        ...
        if purge2:
            debug_print(
                i_clause2,
                clause2.clause_type,
                clause2.clause_string,
                "will be purged. Reason:",
                to_be_purged_reason,
            )
            ...
            debug_print(
                i_clause2, clause2.clause_type, clause2.clause_string, "Not purged"
            )
