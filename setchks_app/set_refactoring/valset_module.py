...
class Clause:
    ...
    def __init__(self, clause_string=None):
        ...
        mObj = re.search(
            r"([^0-9\-]*)([0-9\-]*)$", self.clause_string
        )  # \- minus is to cope with -9999 as the dummy_inactive_concept

        self.clause_operator = mObj.groups()[0]
        # STOPGAP UNTIL CHECK - SET BLANK OPERATOR TO "="
        if self.clause_operator == "":
            print(
                "WARNING: AS STOPGAP MEASURE setting blank operator to '=' in %s"
                % clause_string
            )
    ...
    def __init__(self, clause=None, concepts=None, branch_label=""):
        ...
        if concept is not None:
            if clause.clause_operator == "=":
                ...
                self.members = []
                for descendant_concept_id in [concept.concept_id] + list(
                    concept.descendants
                ):
                    self.members += list(
                        concepts[
                            descendant_concept_id
                        ].modelled_relns_as_destination.keys()
                    )
                self.depth = -9999  # recursive view should currently not show it
            else:
...
class ValsetCollection(list):
    ...
    def append(self, valset=None):
        self.valset_name_to_id[valset.valset_name] = len(
            self
        )  # keep a map of names in the list
...
class Valset:
    ...
    def __init__(
        ...
        if valset_extens_defn != None:
            if valset_extens_defn != set():
                valset_extens_defn = [
                    str(x) for x in valset_extens_defn
        ...
        self.clause_based_rule = ClauseBasedRule(
            clause_set_string=self.clause_set_string
        )
    ...
    def __repr__(self):
        ...
        if self.is_a_filter_valset:
            ...
            repr_strings.append(
                "Filters: %s | %s " % (self.export_filter, self.lexical_filter)
            )
    ...
    def process_metadata(self):
        if self.metadata_column is not None:
            ...
            self.filter_flags = {}
            for ifilter, filter_name in enumerate(
                [
                    "EXCLUDE_LINKAGE_ET_AL",
                    "EXCLUDE_INACTIVE_ET_AL",
                    "OBSOLETE_FILTER",
                    "EXCLUDE_CLINICALLY_UNLIKELY",
                    "GLOBEX",
                    "EXCLUDE_DRUGS_ET_AL",
                ]
            ):
...
class ValsetMembershipAnalysis:
    def __init__(
        ...
        for clause in valset.clause_based_rule.clauses:
            # print("cma start..")
            clause_membership_analysis = ClauseMembershipAnalysis(
            ...
            if clause.clause_type == "exclude":
                full_exclusion_set = full_exclusion_set.union(
                    set(clause_membership_analysis.members)
                )
            else:
                full_inclusion_set = full_inclusion_set.union(
                    set(clause_membership_analysis.members)
                )
        ...
        for i_clause, clause in enumerate(valset.clause_based_rule.clauses):
            if clause.clause_type == "include":
                trimmed_set = set(
                    self.clause_membership_analyses_list[i_clause].members
                ).difference(full_exclusion_set)
            else:
                # trimmed_set=[]
                trimmed_set = set(
                    self.clause_membership_analyses_list[i_clause].members
                ).intersection(full_inclusion_set)
        ...
        if stub_out_interaction_matrix_calc == True:  # fill matrix with zeroes
            ...
            for clause_membership_analysis in self.clause_membership_analyses_list:
                ...
                for clause2_membership_analysis in self.clause_membership_analyses_list:
                    ...
                    if "exclude" in [
                        clause_membership_analysis.clause.clause_type,
                        clause2_membership_analysis.clause.clause_type,
                    ]:
                        n = -n
