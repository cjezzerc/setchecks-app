...
def load_data_into_matrix(
    ...
    setchks_session.table_has_header = (
        table_has_header  # True, False or None(==unknown)
    )
    if (
        setchks_session.table_has_header
    ):  # ** generally need graceful ways to cope with things like a file that has no data rows etc
    ...
    if upload_method in [
        "from_file",
        "from_filename",
    ]:  # from_filename only implemented for xlsx due to issue with zip file error

        if upload_method == "from_file":
            setchks_session.filename = getattr(
                data, "filename", None
            )  # file obj for data passed in via GUI
            # seems to have "filename" attribute
            if (
                setchks_session.filename == None
            ):  # whereas if from data=open(file) has "name"
                setchks_session.filename = getattr(data, "name", None)
        else:
            setchks_session.filename = data

        if (len(setchks_session.filename) >= 6) and (
            setchks_session.filename[-5:] == ".xlsx"
        ):
            file_type = "xlsx"
        else:
            file_type = "other"
        ...
        if file_type == "xlsx":
            ...
            for line in setchks_session.unparsed_data:
                if type(line) == str:
                    ...
                    try:
                        ...
                        decoded_line = str(
                            line, "ISO-8859-1"
                        )  # this is what Excel seems to want to save tab-delimited files as
            ...
            for row in setchks_session.data_as_matrix:
                if len(row) < nmax:
                    row += [
                        DataCellContents(cell_contents="")
                        for x in range(0, nmax - len(row))
                    ]
