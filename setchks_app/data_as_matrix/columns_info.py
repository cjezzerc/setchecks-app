...
class ColumnsInfo:
    __slots__ = [
        "_identified_columns",  # dict of the columns nominated as providing particular info
    ...
    def set_column_type(self, *, icol=None, requested_column_type=None):
        ...
        if (
            requested_column_type in self.identified_columns
            and self._identified_columns[requested_column_type] != None
        ):
            self._column_types[self._identified_columns[requested_column_type]] = (
                "OTHER"
            )
    ...
    def have_just_cid_column(self):
        test = (self._identified_columns["CID"] != None) and (
            self._identified_columns["DID"] == None
        )
        return test
...
if __name__ == "__main__":
    ...
    for actions in [
        ...
        success, message = ci.set_column_type(
            icol=actions[0], requested_column_type=actions[1]
        )
        ...
        print(
            ci.have_just_cid_column,
            ci.have_just_did_column,
            ci.have_both_cid_and_did_columns,
            ci.have_mixed_column,
            ci.have_dterm_column,
        )
