...
def store_summary_dict_to_db(
    ...
    for setchk_code in [x.setchk_code for x in setchks_session.selected_setchks]:
        if (
            setchk_code
            in setchks_session.setchks_run_status  # test needed in case fail gatekeeper
            and setchks_session.setchks_run_status[setchk_code] != "failed"
        ):
            for set_level_table_row in setchks_session.setchks_results[
                setchk_code
    ...
    summary_dict = {}
    summary_dict["Time and date checks were run"] = (
    ...
    summary_dict["Run identifier"] = (
        setchks_session.uuid + ":" + summary_dict["Time and date checks were run"]
    )
    ...
    summary_dict["Number of rows in file"] = setchks_session.first_data_row + len(
