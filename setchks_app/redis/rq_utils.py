...
def start_specific_rq_worker(worker_name=None):
    ...
    if worker_name == "worker_short_jobs":
        os.system(
            f"rq worker --name worker_short_jobs --url '{redis_string}' short_jobs_queue &"
        )
    else:
        os.system(
            f"rq worker --name worker_long_jobs --url '{redis_string}' long_jobs_queue &"
        )
...
def jobs():
    ...
    for q in queues:
        ...
        for job_id in (
            job_ids_in_queue + job_ids_started + job_ids_finished + job_ids_failed
        ):
            try:
                job = Job.fetch(job_id, connection=redis_connection)
                ...
                try:
                    ...
                    data.append(
                        f"{job_id} {status:10} q:{enqueued_at}  s:{started_at}  e:{ended_at} {func}"
                    )
                except:
