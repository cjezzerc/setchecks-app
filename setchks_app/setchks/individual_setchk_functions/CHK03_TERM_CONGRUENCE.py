...
def generate_check_item(
    ...
    if outcome_code == "CHK03-OUT-01":
        ...
        check_item.general_message = "The Preferred Term for this Concept, as identified by the Concept Id, is -->"
        check_item.row_specific_message = f"{preferred_term}"
    # </check_item>
    # <check_item>
        ...
        check_item.row_specific_message = f"{implied_dterm}"
    # </check_item>
    # <check_item>
    ...
    # <check_item>
        ...
        check_item.outcome_level = (
            "Conditional: FACT/DEBUG"  # DEBUG unless n_FSN_FOR_DATA_ENTRY>0
        )
        check_item.general_message = (
        ...
        check_item.row_specific_message = f"{phrase_to_output[dterm_type]}"
    # </check_item>
    # <check_item>
    elif outcome_code == "CHK03-OUT-05":
        if dterm_type == "fsn" and data_entry_extract_type in [
            "ENTRY_PRIMARY",
            "ENTRY_OTHER",
        ]:
            check_item = CheckItem(outcome_code=outcome_code + did_term_type)
    ...
    # <check_item>
        ...
        check_item.row_specific_message = f"{csr_correct_dterm}"
    # </check_item>
    # <check_item>
    elif outcome_code == "CHK03-OUT-08":
        check_item = CheckItem(outcome_code=outcome_code)
        check_item.outcome_level = "FACT"
        check_item.general_message = "The provided Description Id corresponds to the Concept identified by the Concept Id -->"
        check_item.row_specific_message = termbrowser_hyperlink(
            sctid=implied_concept_id
        )
    ...
    # </check_item>
        ...
        check_item = CheckItem(outcome_code=outcome_code)
        check_item.outcome_level = "ISSUE"
        check_item.general_message = "Unrecognized outcome code"
...
def do_check(setchks_session=None, setchk_results=None):
    ...
    for mr in setchks_session.marshalled_rows:
        ...
        if not mr.blank_row:
            ...
            if concept_id is not None:
                ...
                if (
                    setchks_session.columns_info.have_dterm_column is False
                ):  # no DTERM column
                    ...
                    if mr.D_Term_entered == "":  # Term is blank
                        ...
                        if mr.C_Id_entered is not None:  # CID and Term
                            if (
                                not mr.congruence_of_C_Id_entered_and_D_Term_entered_case_insens
                            ):
                            ...
                            if (
                                mr.congruence_of_D_Id_entered_and_D_Term_entered_case_insens
                            ):
                ...
                if mr.D_Term_Type_derived_from_D_Id_entered is not None:
                    ...
                    dterm_type = (
                        mr.D_Term_Type_derived_from_C_Id_entered_and_D_Term_entered
                    )
                else:
                    dterm_type = ""
                ...
                if mr.D_Term_derived_from_D_Id_entered is not None:
                    ...
                    csr_correct_dterm = (
                        mr.D_Term_csr_correct_derived_from_C_Id_entered_and_D_Term_entered
                    )
                else:
                    csr_correct_dterm = ""
                ...
                for outcome_code_digits in outcome_codes_matrix[leaf]:
                    ...
                    if outcome_code_digits == "03" and mr.D_Id_active == "0":
                        ...
                        if (
                            dterm_type == "fsn"
                            and setchks_session.data_entry_extract_type
                            in ["ENTRY_PRIMARY", "ENTRY_OTHER"]
                        ):
        ...
        def sorting_order(check_item):
            trimmed_outcome_code = check_item.outcome_code[
                :12
            ]  # get rid of final a,b,c
            order = [
                "CHK03-OUT-03",
                "CHK03-OUT-05",
                "CHK03-OUT-06",
    ...
    if n_ISSUES != 0:
        ...
        if n_INACTIVE_DESCRIPTION != 0:
            ...
            setchk_results.set_level_table_rows.append(
                SetLevelTableRow(
                    descriptor=("Number of rows with inactive Descriptions"),
        ...
        if n_CAPITALISATION_ISSUE != 0:
            ...
            setchk_results.set_level_table_rows.append(
                SetLevelTableRow(
                    descriptor=("Number of rows where there is a capitalisation issue"),
    ...
    if n_MISSING_TERM != 0:
        ...
        setchk_results.set_level_table_rows.append(
            SetLevelTableRow(
                descriptor=("Number of rows where the Term is missing"),
