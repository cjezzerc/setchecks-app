...
def do_check(setchks_session=None, setchk_results=None):
    ...
    setchks_session.terminology_server = (
        setchks_app.terminology_server_module.TerminologyServer()
    )
    refset_response = setchks_session.terminology_server.do_expand(
        ecl=ecl,
    ...
    for i_row, row in enumerate(
        ...
        if active_status is True:
            ...
            check_item["Message"] = (
                "Concept could not be found in the specified release"
            )
            n_not_found += 1
    ...
    #                           #
    setchk_results.set_analysis["Message"] = (
        "The value set has %s inactive concepts (tot=%s;active=%s;not_found=%s)"
        % (n_inactive, n_processed, n_active, n_not_found)
    )
    #                           #
    #############################
