...
def analyse_hst_data(hst_data=None):
    if (
        len(hst_data) == 0
    ):  # should not really happen but this would be if e.g. passed in hst_data for an active concept
        return None, None
...
class SuppTabRow:
    ...
    def __init__(self):
        ...
        self.implied_concept_id = (
            ""  # null string so that if not set it shows as a blank
        )
        self.term = None
        self.eff_time = None
...
def do_check(setchks_session=None, setchk_results=None):
    ...
    if not dual_mode:
        ...
        concepts = ConceptsDict(sct_version=sct_version.date_string)
        concepts_earlier_sct_version = ConceptsDict(
            sct_version=earlier_sct_version.date_string
        )
    ...
    setchk_results.supp_tab_blocks = (
        []
    )  # there will be one entry in list for each data row
    ...
    for i_data_row, mr in enumerate(setchks_session.marshalled_rows):
        if mr.C_Id is not None:
            ...
            if dual_mode:
                active_status_earlier_sct_release[concept_id] = (
                    concepts_earlier_sct_version[concept_id].active
                )
            if active_status[concept_id] is False:
                ...
                if (not dual_mode) or active_status_earlier_sct_release[
                    concept_id
                ] is True:
                    if interpretation == "NO_REPLACEMENT":
                        ...
                        for i_option, hst_option in enumerate(hst_options):
                            ...
                            supp_tab_row.file_row_number = (
                                setchks_session.first_data_row + i_data_row + 1
                            )
                            ...
                            supp_tab_row.replacement_option_counter = (
                                f"{i_option+1}/{len(hst_options)}"
                            )
                            supp_tab_row.replacement_concept_id = (
                                hst_option.new_concept_id
                            )
                            supp_tab_row.replacement_concept_pt = concepts[
                                hst_option.new_concept_id
                            ].pt
                            ...
                            supp_tab_row.is_replacement_concept_in_set = (
                                supp_tab_row.replacement_concept_id in valset_members
                            )
                            if supp_tab_row.is_replacement_concept_in_set is True:
                                supp_tab_row.is_correct_representation_type_in_set = (
                                    supp_tab_row.id_type
                                    in valset_representations_dict[mr.C_Id]
                                )
                            else:
                                supp_tab_row.is_correct_representation_type_in_set = "-"
                            ...
                            if interpretation == "UNAMBIGUOUS":
                                ...
                                if supp_tab_row.is_replacement_concept_in_set is True:
                                    N_CONCEPTS_WITH_UNAMBIG_REPL_WITH_REPL_IN_VALSET += (
                                        1
                                    )
        ...
        if not mr.blank_row:
            ...
            if concept_id is not None:
                ...
                if active_status[concept_id] is True:  # "CHK04-OUT-i"
                    ...
                    check_item.general_message = "Concept is active"
                    # </check_item>
    ...
    if not dual_mode:
        if n_CONCEPTS_INACTIVE == 0:
            ...
            setchk_results.set_level_table_rows.append(
                SetLevelTableRow(
                    descriptor=("Number of inactive Concepts "),
            ...
            setchk_results.set_level_table_rows.append(
                SetLevelTableRow(
                    descriptor=("Number of inactive Concepts with no replacement "),
