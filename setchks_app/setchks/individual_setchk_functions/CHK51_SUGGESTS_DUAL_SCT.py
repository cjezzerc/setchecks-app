...
from setchks_app.set_refactoring.valset_module import (
...
def do_check(setchks_session=None, setchk_results=None):
    ...
    ):  # marshalled_rows is built based on info from setchks_session.sct_version
    ...
    for clause in clauses:
        concept_ids_earlier = ClauseMembershipAnalysis(
            clause=clause, concepts=concepts_earlier
        ).members  # ".members" is a list of concept ids
        concept_ids_later = ClauseMembershipAnalysis(
            clause=clause, concepts=concepts_later
        ).members  # ".members" is a list of concept ids
        members_earlier = set(
            concepts_earlier[x] for x in concept_ids_earlier
        )  # now members is a set of Concepts
        members_later = set(
            concepts_later[x] for x in concept_ids_later
        )  # now members is a set of Concepts
        if clause.clause_type == "include":
            include_clauses_and_memberships.append(
                (clause, members_earlier, members_later)
            )
        else:
            exclude_clauses_and_memberships.append(
                (clause, members_earlier, members_later)
            )
    ...
    whole_vs_concept_ids_earlier = set(
        ValsetMembershipAnalysis(
            valset=refactored_valset, concepts=concepts_earlier
        ).final_inclusion_list
    )
    whole_vs_concept_ids_earlier = set(str(x) for x in whole_vs_concept_ids_earlier)

    assert (
        whole_vs_concept_ids_earlier == value_set_members
    )  # belt and braces check that refactoring retained membership

    whole_vs_concept_ids_later = set(
        ValsetMembershipAnalysis(
            valset=refactored_valset, concepts=concepts_later
        ).final_inclusion_list
    )
    whole_vs_concept_ids_later = set(str(x) for x in whole_vs_concept_ids_later)

    whole_vs_concept_ids_in_common = whole_vs_concept_ids_earlier.intersection(
        whole_vs_concept_ids_later
    )
    whole_vs_concept_ids_only_in_earlier = whole_vs_concept_ids_earlier.difference(
        whole_vs_concept_ids_later
    )
    whole_vs_concept_ids_only_in_later = whole_vs_concept_ids_later.difference(
        whole_vs_concept_ids_earlier
    )
    ...
    for sorting_flag in [
        "ONLY_NON_ZERO",
        "ONLY_ZERO",
    ]:  # sorting_flag float include clauses with something in earlier
        # or later column to top as those are most interesting
        for i_clause, clause_and_members_tuple in enumerate(
            include_clauses_and_memberships
        ):
            include_clause, include_members_earlier, include_members_later = (
                clause_and_members_tuple
            )
            members_in_vs_from_this_clause_earlier = include_members_earlier.difference(
                all_excluded_concepts_earlier
            )
            members_in_vs_from_this_clause_later = include_members_later.difference(
                all_excluded_concepts_later
            )
            ...
            members_in_vs_from_this_clause_earlier_dict = {
                m.concept_id: m for m in members_in_vs_from_this_clause_earlier
            }
            concept_ids_in_vs_from_this_clause_earlier = set(
                members_in_vs_from_this_clause_earlier_dict.keys()
            )
            members_in_vs_from_this_clause_later_dict = {
                m.concept_id: m for m in members_in_vs_from_this_clause_later
            }
            concept_ids_in_vs_from_this_clause_later = set(
                members_in_vs_from_this_clause_later_dict.keys()
            )

            # now do set operations on concept_id sets
            concept_ids_in_vs_from_this_clause_common = (
                concept_ids_in_vs_from_this_clause_earlier.intersection(
                    concept_ids_in_vs_from_this_clause_later
                )
            )
            concept_ids_in_vs_from_this_clause_only_earlier = (
                concept_ids_in_vs_from_this_clause_earlier.difference(
                    concept_ids_in_vs_from_this_clause_later
                )
            )
            concept_ids_in_vs_from_this_clause_only_later = (
                concept_ids_in_vs_from_this_clause_later.difference(
                    concept_ids_in_vs_from_this_clause_earlier
                )
            )

            # now build member sets back from member dicts and concept_id sets
            members_in_vs_from_this_clause_common = set(
                members_in_vs_from_this_clause_earlier_dict[cid]
                for cid in concept_ids_in_vs_from_this_clause_common
            )
            members_in_vs_from_this_clause_only_earlier = set(
                members_in_vs_from_this_clause_earlier_dict[cid]
                for cid in concept_ids_in_vs_from_this_clause_only_earlier
            )
            members_in_vs_from_this_clause_only_later = set(
                members_in_vs_from_this_clause_later_dict[cid]
            ...
            n_members_of_clause_in_vs_common = len(
                members_in_vs_from_this_clause_common
            )
            n_members_of_clause_in_vs_only_earlier = len(
                members_in_vs_from_this_clause_only_earlier
            )
            n_members_of_clause_in_vs_only_later = len(
                members_in_vs_from_this_clause_only_later
            )
            ...
            n = (
                n_members_of_clause_in_vs_only_earlier
                + n_members_of_clause_in_vs_only_later
            )
            do_output_this_loop = (
                n == 0
                and sorting_flag == "ONLY_ZERO"
                and setchks_session.output_full_or_compact == "FULL_OUTPUT"
            ) or (n > 0 and sorting_flag == "ONLY_NON_ZERO")
            if do_output_this_loop:
                ...
                plain_english_formatted_clause = plain_english_operators_fmts[
                    include_clause.clause_operator
    ...
    if (
        len(whole_vs_concept_ids_only_in_earlier)
        + len(whole_vs_concept_ids_only_in_later)
    ) == 0:
        # <set_level_message>
