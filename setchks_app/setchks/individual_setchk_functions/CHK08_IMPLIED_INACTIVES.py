...
def do_check(setchks_session=None, setchk_results=None):
            ...
            if active_status[concept_id] is True:
                ...
                if hst_options:
                    for i_option, hst_option in enumerate(hst_options):
                        ...
                        supp_tab_row.implied_inactive_option_counter = (
                            f"{i_option+1}/{len(hst_options)}"
                        )
                        supp_tab_row.implied_inactive_concept_id = (
                            hst_option.old_concept_id
                        )
                        supp_tab_row.implied_inactive_concept_pt = concepts[
                            hst_option.old_concept_id
                        ].pt
                        supp_tab_row.implied_inactive_concept_eff_time = concepts[
                            hst_option.old_concept_id
                        ].effective_time
                        supp_tab_row.ambiguity_status = hst_option.is_ambiguous
                        supp_tab_row.is_implied_inactive_concept_in_set = (
                            supp_tab_row.implied_inactive_concept_id in valset_members
                        )
        ...
        if not mr.blank_row:
            ...
            if concept_id is not None:
                ...
                if active_status[concept_id] is False:
                    ...
                    check_item.general_message = "This Concept is inactive"
                    # </check_item>
    ...
    setchk_results.set_level_table_rows.append(
        SetLevelTableRow(
            descriptor=("Number of rows with an inactive Concept"),
