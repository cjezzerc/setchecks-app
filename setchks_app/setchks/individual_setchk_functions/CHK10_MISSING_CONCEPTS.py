...
def do_check(setchks_session=None, setchk_results=None):
    ...
    for clause in clauses:
        members = ClauseMembershipAnalysis(
        ...
        if clause.clause_type == "include":
            ...
            setchk_results.chk_specific_data["include_clauses"].append(
                (clause, len(members))
            )
        else:
            ...
            setchk_results.chk_specific_data["exclude_clauses"].append(
                (clause, len(members))
            )
    ...
    for sorting_flag in [
        "ONLY_NON_ZERO",
        "ONLY_ZERO",
    ]:  # sorting_flag float include clauses with some
        ...
        for i_clause, clause_and_members_tuple in enumerate(
            ...
            members_in_vs_from_this_clause = include_members.difference(
                all_excluded_concepts
            )
            members_excluded_from_this_clause = include_members.intersection(
                all_excluded_concepts
            )
            ...
            ) or (n > 1 and sorting_flag == "ONLY_NON_ZERO")
            if do_output_this_loop:
                ...
                for ei_clause, e_clause_and_members_tuple in enumerate(
                    exclude_clauses_and_memberships
                ):
                    exclude_clause, exclude_members = e_clause_and_members_tuple
                    members_of_include_that_this_exclude_removes = (
                        members_excluded_from_this_clause.intersection(exclude_members)
                    )
                    ...
                    if members_of_include_that_this_exclude_removes != set():
                        if len(members_of_include_that_this_exclude_removes) > 1:
                            ...
                            plain_english_formatted_clause = (
                                plain_english_operators_fmts[
                                    exclude_clause.clause_operator
                                ]
                ...
                for member in members_in_vs_from_this_clause:
                    row = chk_specific_sheet.new_row()
                    row.cell_contents = ["", "", str(member.concept_id), member.pt]
    ...
    if n_SUGGESTED_NEW_MEMBERS == 0:
        ...
        setchk_results.set_level_table_rows.append(
            SetLevelTableRow(
                descriptor=("Number of suggestions made"),
