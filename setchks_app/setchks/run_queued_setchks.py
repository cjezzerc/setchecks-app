...
def run_queued_setchks(setchks_list=None, setchks_session=None, run_in_rq=True):
    ...
    setchks_session.setchks_to_run_as_gatekeeper_not_passed = [
        "CHK02_IDS_IN_RELEASE",
        "CHK20_INCORR_FMT_SCTID",
    ]
    if setchks_session.data_entry_extract_type == "EXTRACT":
        setchks_session.setchks_to_run_as_gatekeeper_not_passed.append(
            "CHK01_APPROP_SCTID"
        )
    ...
    for setchk in setchks_list:
        if run_in_rq:
            if (
                setchks_session.passes_gatekeeper
                or setchk.setchk_code
                in setchks_session.setchks_to_run_as_gatekeeper_not_passed
            ):
                logger.debug(
                    f"ABOUT TO SEND TO RQ: {setchk.setchk_short_name_plus_short_code}"
                )
                setchks_session.setchks_jobs_manager.launch_job(
                    setchk=setchk,
                    setchks_session=setchks_session,
                )
                logger.debug(f"SENT TO RQ: {setchk.setchk_code}")
    ...
    if run_in_rq:
        job_status_report = setchks_session.setchks_jobs_manager.update_job_statuses()
        logger.debug("\n".join(job_status_report))
