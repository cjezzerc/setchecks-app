...
class SetchksSession:
    ...
    * a means to execute the tests(via rq?), and (TBC) a means to monitor progress
    ...
    def __init__(self):
        ...
        self.column_content_assessment = (
            column_content_assessment.ColumnContentAssessment()
        )
        ...
        self.timings = (
            {}
        )  # arbitrary entries can be put in this dict for debug purposes
    ...
    def load_data_into_matrix(
        self, data=None, upload_method=None, table_has_header=None, separator="\t"
    ):

        return load_data_into_matrix.load_data_into_matrix(
            self,
            data=data,
    ...
    def set_sct_versions(self):
        ...
        self.available_sct_versions = current_app.config[
            "sct_versions_available_in_app"
        ]
        ...
        for mr in self.marshalled_rows:
            ...
            if self.passes_gatekeeper:
                if mr.C_Id is None and not mr.blank_row:
                    ...
                    self.data_entry_extract_type == "EXTRACT"
                    and mr.D_Id_entered is not None
                ):
                    self.passes_gatekeeper = False
        self.preprocessing_done = True
        return (
            self.marshalled_rows,
            self.preprocessing_done,
            self.passes_gatekeeper,
        )  # this is for if run in a queue
