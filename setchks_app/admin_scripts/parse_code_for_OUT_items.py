        ...
        for line in open(filename_plus_path).readlines():
            ...
            if f and f[0] == "#<" + output_type + ">":
                if printing_on is True:
                    print(
                        "Error: printing_on is already True; structured comment error?"
                    )
                    sys.exit()
                printing_on = True
                lines = []
                if not output_tsv:
                    print(
                        f"------------ {filename} line: {i_line} -----{output_type}--------------------"
                    )
            if printing_on:
                lines.append(line[:-1])
                if not output_tsv:
            ...
            if f and f[0] == "#</" + output_type + ">":
                if printing_on is False:
                    print(
                        "Error: printing_on is already False; structured comment error?"
                    )
                    sys.exit()
                printing_on = False
