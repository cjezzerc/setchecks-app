...
def make_analysis_by_row_sheet(
    ...
    if (
        ws is None
    ):  # if no ws passed in then the routine is just precalculating the row map
        ...
        ws = (
            []
        )  # but with ws as a regular list the ws.append lines do not need to be stubbed out
    else:
        creating_ws = True
    ...
    row_analysis_row_numbers_map = (
        []
    )  # each entry in list corresponds 1:1 to a row in data file
    ...
    for i_data_row, data_row in enumerate(
        ...
        for i_col, cell_content in enumerate(data_row):
            if (
                i_col == mixed_column and cid is not None and cid_entered is not None
            ):  # don't hyperlink D_Id
                # or non-SCTID
        ...
        for setchk_code in setchks_list_to_report:
            ...
            if setchk_results.row_analysis != []:
                if (
                    setchk_code in supp_tabs_row_numbers_map
                ):  # i.e. there is a supp tab for this check
                    supp_tab_ws, supp_tab_mapping = supp_tabs_row_numbers_map[
                        setchk_code
                    ]
                ...
                outcome_codes_count = (
                    {}
                )  # this is used to make sure that in the case where the same outcome_code
                ...
                for check_item in setchk_results.row_analysis[i_data_row]:
                    if output_OK_messages or check_item.outcome_level not in [
                        "INFO",
                        "DEBUG",
                    ]:
                        ...
                        if creating_ws:
                            row_to_link_to = analysis_by_outcome_row_numbers_map[
                                outcome_code
                            ][i_data_row][outcome_codes_count[outcome_code]]
                        else:
                            row_to_link_to = None

                        # message=f"{outcome_code}:{check_item.general_message}"
                        message_type = styling.message_types_text[
                        ...
                        hyperlink_cell_contents = (
                            f'=HYPERLINK("#Grp_by_Message!G{row_to_link_to}","M")'
                        )
                        ...
                        ws_append_time += dt
                        if (
                            setchk_short_code != previous_setchk_short_code
                        ) and something_was_output:
        ...
        for i_row, row in enumerate(ws.iter_rows()):
            ...
            for i_cell, cell in enumerate(row):
                if i_row == 0:
                    ...
                    if (
                        len(strval) >= 16
                        and str(cell.value)[0:16] == '=HYPERLINK("http'
                    ):
